  ...
  todos.forEach((todo) => {
    todos_list_body.innerHTML += `
            <tr class="todo-item" data-id="${todo.id}">
                <td>${todo.task}</td>
                <td>${todo.dueDate ? formatDate(todo.dueDate) : "No due date"}</td>
                <td>${todo.status}</td>
                <td>
...
function toggleStatus(id) {
  ...
  if (todo.status === "pending") {
    ...
    todo.completed = true; // Update the completed status
  } else {
    todo.status = "pending";
    todo.completed = false; // Update the completed status
  }

  saveToLocalStorage();
  filterTodos("all"); // Refresh the filtered view to update the display
}
